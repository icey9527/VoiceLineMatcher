...
<head>
    ...
    <style>
        ...
        .search-box { margin-bottom: 20px; padding: 15px; background-color: #f5f5f5; border-radius: 8px; }
        .search-row { display: flex; gap: 10px; margin-bottom: 10px; }
        input[type="text"] { flex: 1; padding: 8px 12px; border: 2px solid #ddd; border-radius: 4px; }
        button[type="submit"] { padding: 8px 20px; background: #2196F3; color: white; border: none; border-radius: 4px; cursor: pointer; }
        .search-types { display: flex; gap: 8px; }
        .type-btn { padding: 6px 15px; border: 2px solid #2196F3; border-radius: 15px; background: white; color: #2196F3; cursor: pointer; transition: all 0.3s; font-size: 0.9em; }
        .type-btn.active { background: #2196F3; color: white; }
        .sentence-list { list-style: none; padding: 0; }
        .sentence-item { display: flex; align-items: center; gap: 15px; padding: 12px; border-bottom: 1px solid #eee; cursor: pointer; transition: background 0.2s; -webkit-touch-callout: none; -webkit-user-select: none; user-select: none; }
        .sentence-item:hover { background-color: #f8f8f8; }
        .info-block { flex: 0 0 90px; display: flex; flex-direction: column; gap: 5px; }
        .speaker-badge, .hash-badge { padding: 3px 8px; border-radius: 3px; font-size: 0.75em; line-height: 1.3; }
        .speaker-badge { background: #2196F3; color: white; }
        .hash-badge { background: #4CAF50; color: white; word-break: break-all; }
        .sentence-text { flex-grow: 1; font-size: 0.95em; }
        .audio-player { display: none; }
        .no-results { color: #666; padding: 20px; text-align: center; }
        .clear-btn { padding: 8px 15px; background: #eee; color: #666; text-decoration: none; border-radius: 4px; }
        .pagination { margin: 20px 0; text-align: center; }
        .pagination a { display: inline-block; padding: 8px 12px; margin: 0 3px; background: #f0f0f0; border-radius: 4px; text-decoration: none; }
        .pagination a.active { background: #2196F3; color: white; }
        .search-hint { font-size: 0.8em; color: #666; text-align: center; margin: 10px 0; opacity: 0.8; }
        .page-selector { padding: 8px 12px; border: 2px solid #2196F3; border-radius: 4px; background: white; color: #2196F3; font-size: 1em; cursor: pointer; outline: none; width: 150px; text-align: center; }
        .page-selector:hover { background: #f5f5f5; }
        .page-selector option { padding: 5px; }
    </style>
</head>
<body>
    <h1><?= $t['title'] ?></h1>
        ...
        <form method="get" action="">
            ...
            <div class="search-row">
                ...
                <?php if (!empty($searchTerm)): ?>
                    <a href="?lang=<?= $lang ?>" class="clear-btn"><?= $t['clear_button'] ?></a>
                <?php endif; ?>
            </div>
            <div class="search-types">
                <div class="type-btn <?= $searchType === 'sentence' ? 'active' : '' ?>" 
                     onclick="setSearchType('sentence')"><?= $t['sentence_tab'] ?></div>
                <div class="type-btn <?= $searchType === 'speaker' ? 'active' : '' ?>" 
                     onclick="setSearchType('speaker')"><?= $t['speaker_tab'] ?></div>
    ...
    <?php if (empty($filteredSentences)): ?>
        <li class="no-results"><?= $t['no_results'] ?></li>
    <?php else: ?>
...
<?php if ($totalPages > 1): ?>
    <div class="pagination" style="
        ...
        <button onclick="goToPage(<?= $page - 1 ?>)" 
                <?= $page <= 1 ? 'disabled' : '' ?>
                class="page-button" title="Previous">&lt;</button>
        <!-- 分页选择器 -->
        ...
        <button onclick="goToPage(<?= $page + 1 ?>)" 
                ...
                class="page-button" title="Next">&gt;</button>
        <!-- 每页数量选择器 -->
    ...
    <style>
        ...
        .page-button:hover:not(:disabled) { background: #2196F3; color: white; }
    ...
    <script>
        ...
        function playAudio(id) {
            ...
            player.play().catch(error => {
                player.muted = true;
                player.play().then(() => { player.muted = false; });
            });
        }
        // 智能事件处理
        document.querySelectorAll('.sentence-item').forEach(item => {
            ...
            item.addEventListener('touchstart', function(e) {
                ...
                isScrolling = false;
                pressTimer = setTimeout(() => { handleLongPress(id); }, 600);
            }, { passive: true });
            ...
            item.addEventListener('mousedown', function() {
                mouseTimer = setTimeout(() => { handleLongPress(id); }, 600);
            });
